...
public class ProfileManagerController extends AbstractPokerClientController {
	...
	@FXML protected void handleModify(ActionEvent event) {
		oldPasswordField.getStyleClass().remove(ERR_STYLECLASS);
		newPasswordField.getStyleClass().remove(ERR_STYLECLASS);
		...
		if (!newPassword.equals(rePassword) ||
				...
				newPassword.equals("") ||
				rePassword.equals("")) {
			newPasswordField.getStyleClass().add(ERR_STYLECLASS);
			...
			} catch (PokerInvalidPassword e) {
				oldPasswordField.getStyleClass().add(ERR_STYLECLASS);
				showErrorAlert(e.getMessage());
			}
