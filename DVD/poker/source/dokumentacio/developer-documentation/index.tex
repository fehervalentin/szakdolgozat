...
\begin{Verbatim}[xleftmargin=.5in]
 ...
 mvn site:stage
\end{Verbatim}
utasításokkal. Az elkészült dokumentációk a \path{\target\staging} könyvtárba kerülnek. Kettő darab \texttt{index.html} állományt kell keresnünk, az egyik a gyökérkönyvtárban található a másik pedig az \path{apidocs} könyvtár gyökerében.
...
\section{Adatbázisséma} \label{sec:db-schema}
\begin{figure}[h!]
  \caption{Adatbázisséma}
  \label{fig:db_scheme}
  \centering
...
\subsection{Probléma megfogalmazása}
...
\subsection{poker-model modul} \label{sec:poker-model}
...
\end{Verbatim}
parancsot kiadva a \path{poker\release} könyvtár alá csomagolódik be a futtatható jar állomány.
...
\subsection{poker-server modul}
\begin{figure}[h!]
  \caption{A szerver publikus interfésze}
  \label{fig:server_public}
  \centering
    \includegraphics[width=\textwidth]{developer-documentation/images/server_remote2.png}
\end{figure}
...
Az összecsomagoláshoz igénybe vehetjük a mavent és annak az \texttt{assembly pluginját}. A poker-server könyvtárban a
 \begin{Verbatim}[xleftmargin=.5in]
mvn clean compile assembly:single
\end{Verbatim}
parancsot kiadva a \path{poker\release} könyvtár alá csomagolódik be a futtatható jar állomány.
...
A felhasználók egyenlegét adminisztrációs joggal rendelkező felhasználó vissza tudja állítani alaphelyzetbe, amely 10.000 egységnek felel meg.
