...
public class CreateTableController extends AbstractPokerClientController {
	...
	private final String ERR_BLIND_MSG = "A nagy vaknak nagyobbnak kell lennie, mint 0!";
	...
	@FXML protected void createTableHandler(ActionEvent event) {
		...
		if (bigBlind.compareTo(BigDecimal.ZERO) <= 0) {
			showErrorAlert(String.format(ERR_BLIND_MSG));
			return;
		}
